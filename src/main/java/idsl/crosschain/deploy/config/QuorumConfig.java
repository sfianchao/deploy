package idsl.crosschain.deploy.config;
...
public class QuorumConfig {
    ...
    @Value("${quorum.chain.src.ip}")
    private String srcChainIp;

    @Value("${quorum.chain.relay.ip}")
    private String relayChainIp;

    @Value("${quorum.chain.dest.ip}")
    private String destChainIp;
...
//        try {
//            // connect to blockchain
//            Quorum quorum = Quorum.build(new HttpService("http://140.118.9.226:23001"));
//            Web3ClientVersion web3ClientVersion = quorum.web3ClientVersion().send();
//            String clientVersion = web3ClientVersion.getWeb3ClientVersion();
...
//        try {
//            // connect to blockchain
//            Quorum quorum = Quorum.build(new HttpService("http://140.118.9.225:23001"));
//            Web3ClientVersion web3ClientVersion = quorum.web3ClientVersion().send();
//            String clientVersion = web3ClientVersion.getWeb3ClientVersion();
//            log.info("[src chain] client version: " + clientVersion);
//
//            // set up credentials
//            Credentials credentials = Credentials.create("d4659c3f7888bf8a5d3e2313f9e79fb141ab408f4f217adef18190ed98a2fff5");
//            StaticGasProvider gasProvider = new StaticGasProvider(BigInteger.valueOf(20000000000L), BigInteger.valueOf(6721975L));
...
//        try {
//            // connect to blockchain
//            Quorum quorum = Quorum.build(new HttpService("http://140.118.9.227:23001"));
//            Web3ClientVersion web3ClientVersion = quorum.web3ClientVersion().send();
//            String clientVersion = web3ClientVersion.getWeb3ClientVersion();
//            log.info("[dest chain] client version: " + clientVersion);
//
...
//    --------------------------------------------------------------------------------------------------------------------
    ...
    public QuorumInfo quorumBuilder() {
        ...
        try {
            // connect to blockchain
            Quorum quorum = Quorum.build(new HttpService(relayChainIp));
            Web3ClientVersion web3ClientVersion = quorum.web3ClientVersion().send();
            String clientVersion = web3ClientVersion.getWeb3ClientVersion();
            log.info("client version: " + clientVersion);
    ...
    public QuorumInfo sourceChainBuilder() {
        ...
        try {
            // connect to blockchain
            Quorum quorum = Quorum.build(new HttpService(srcChainIp));
            Web3ClientVersion web3ClientVersion = quorum.web3ClientVersion().send();
            String clientVersion = web3ClientVersion.getWeb3ClientVersion();
            log.info("[src chain] client version: " + clientVersion);
    ...
    public QuorumInfo destinationChainBuilder() {
        ...
        try {
            // connect to blockchain
            Quorum quorum = Quorum.build(new HttpService(destChainIp));
            Web3ClientVersion web3ClientVersion = quorum.web3ClientVersion().send();
            String clientVersion = web3ClientVersion.getWeb3ClientVersion();
            log.info("[dest chain] client version: " + clientVersion);
